...
const existingIndex = cart.items.findIndex(
  (i) =>
    ...
    i.modifierFingerprint ===
      buildModifierFingerprint(dto.selectedOptions ?? []),
);
```
...
if (!group) {
  throw new BadRequestException(
    `Modifier group ${sel.groupId} does not exist...`,
  );
}
const option = group.options.find((o) => o.optionId === sel.optionId);
if (!option) {
  throw new BadRequestException(
    `Modifier option ${sel.optionId} does not exist...`,
  );
}
```
...
```json
{
  "selectedOptions": [{ "groupId": "size-uuid", "optionId": "large-uuid" }]
}
```
...
_Audit conducted against commit state as of 2026-04-29. All line references are approximate and should be verified against the current HEAD._
